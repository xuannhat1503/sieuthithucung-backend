...
class AuthController extends Controller
{
    ...
    protected function frontendAccountUrl(array $query = []): string
    {
        $baseUrl = rtrim(env('FRONTEND_URL', 'http://localhost/sieuthithucung/sieuthithucung-frontend'), '/');
        $accountUrl = $baseUrl . '/pages/account.html';

        if (!empty($query)) {
            $accountUrl .= '?' . http_build_query($query);
        }
    ...
    public function apiLogin(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'redirect_url' => $this->frontendAccountUrl([
                'login' => 'success',
            ]),
            'user' => [
                'id' => $user->id,
