...
class CatalogController extends Controller
{
    ...
    public function show($slug)
    {
        ...
        $related = Product::query()
            ...
            ->map(function (Product $item) {
                return $this->mapProductCard($item);
            })
            ->values();
        ...
        return response()->json([
            'data' => [
                ...
                'short_description' => Str::limit((string) $product->description, 140),
                ...
                'images' => $product->images->map(function ($image) {
                    return $this->normalizeImagePath($image->image);
                })->filter()->values(),
    ...
    protected function mapProductCard(Product $product)
    {
        return [
            'id' => $product->id,
            'name' => $product->name,
    ...
    protected function extractPrimaryImage(Product $product)
    {
    ...
    protected function normalizeImagePath($path)
    {
        ...
        if (Str::startsWith($path, ['/storage/', '/images/', '/uploads/'])) {
            return URL::to($path);
        }
